...
In this article, we will show you how to make migration easier by using some command line tools such as [`git`](https://git-scm.com/), [`cargo`](https://doc.rust-lang.org/cargo/) and [`ast-grep`](/). These tools can help you track the changes, search for specific patterns in your code, and automate API migration. Hope you can migrate your Bevy projects with less hassle and more confidence by following our tips.
...
Before we start, we need to make sure that we have the following tools installed: [Rust](https://rustup.rs/), [git](https://git-scm.com/) and [ast-grep](/).
...
So we just need to change the import name. [Using ast-grep is trivial here](/guide/introduction#introduction).
...
`$STAGE` and `$OWN_STAGE` are [meta-variables](/guide/pattern-syntax#meta-variable).
